...
import { AUTH_KEY, NOTICE, PAGING, URL } from "../../../config/constants";
...
export default function List() {
    ...
    useEffect(()=>{
        const fetch=async ()=>{
            try{
                ...
                const res = await axios.get(`${URL.NOTICE}?_sort=-postDate&_page=${current}&_per_page=${PAGING.PAGE_SIZE}`);//-는 내림 차순
                console.log('=========List.jsx============')
                console.log(res.data.data);
                //dispatch호출
                dispatch({type:NOTICE.ALL,notices:res.data.data});
                //<선택 사항>:로딩 화면 숨기기
                setLoading(false);
            }
            catch(e){
    ...
    //총 게시글 수 변경
    useEffect(()=>{
        axios.get(URL.NOTICE).then(res=>dispatch({type:NOTICE.TOTAL,total:res.data.length}));
    },[]);
    ...
    return <>
            ...
            <tbody>
                {
                    notices.map(notice => (
                        <tr key={notice.id}>
                            ...
                            {/* Link의 state속성으로 notice를 넘기고 받는 쪽에서는 useLocation()훅으로 받는다*/}
                            {/* 상세보기에서 아이디로 조회하는 네트웍 요청 불필요 */}
                            <td><Link to={`/notices/${notice.id}`} state={notice}>{notice.title}</Link></td>
                            {/* ?.  연산자로 계속 새로고침 아이콘 클릭시 오류 방지 */}                            
                           
                            <td className="col-2">{users.find(user => user.username === notice.username)?.name}</td>
                            <td className="col-2">{notice.postDate.substring(0, 20)}</td>
                            <td className="col-1">{notice.views}</td>
                        </tr>
                    ))
                }
                {/* 배열이 비어있을 때  */}
        ...
        <div className="d-flex justify-content-center">
            ...
            <ul className="rc-pagination">
                ...
                } onClick={(e)=>current !==1 ? dispatch({type:NOTICE.CURRENT,current:1}):null } >&laquo;</span>
                </li>        
            </ul>
            ...
            <Pagination 
                ...
                onChange={(current,pageSize)=>{
                    ...
                    dispatch({type:NOTICE.CURRENT,current});
                }}
                locale={locale}
            ...
            <ul className="rc-pagination ms-2">
                ...
                } onClick={(e)=>current !==lastPage ? dispatch({type:NOTICE.CURRENT,current:lastPage}):null } >&raquo;</span>
                </li>
            </ul>
        </div>
    </>
