...
export default function IUpdateForm(){
    ...
    const handleInputs=(e)=>{
        ...
        if(name.toUpperCase()==='TITLE'){//제목 입력시
            if(value.trim().length===0){
                spanTitle.current.textContent='제목을 입력하세요...';
                editButton.current.disabled=true;
            }
            else{
                spanTitle.current.textContent='';
                editButton.current.disabled=false;
            }
        }
        else{//내용 입력시
            if(value.trim().length===0){
                spanContent.current.textContent='내용을 입력하세요...';
                editButton.current.disabled=true;
            }
            else{
                spanContent.current.textContent='';
                 editButton.current.disabled=false;
            }
        }
        //입력값(제목 혹은 내용) 세터로 변경
    ...
    const handleUpdate=e=>{
        ...
        axios
            .put(`${URL.INQUIRY}/${state.id}`,{...state,title,content})
            .then(res=>navigate(`/inquiry/${state.id}`,{state:{...state,title,content}}));
